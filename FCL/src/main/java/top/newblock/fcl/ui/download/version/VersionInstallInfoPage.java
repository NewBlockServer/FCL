package top.newblock.fcl.ui.download.version;
...
public class VersionInstallInfoPage extends FCLTempPage implements View.OnClickListener {
        ...
        for (InstallerItem library : group.getLibraries()) {
            ...
            library.action.set(() -> {
                if (LibraryAnalyzer.LibraryType.FABRIC_API.getPatchId().equals(libraryId)) {
                    ...
                    builder.setNegativeButton(getContext().getString(top.newblock.fcl.R.string.dialog_positive), null);
                    builder.create().show();
                }
                        ...
                        public void onStop(boolean success, TaskExecutor executor) {
                            Schedulers.androidUIThread().execute(() -> {
                                if (success) {
                                    ...
                                    builder1.setNegativeButton(getContext().getString(top.newblock.fcl.R.string.dialog_positive), () -> DownloadPageManager.getInstance().dismissCurrentTempPage());
    ...
    public static void alertFailureMessage(Context context, Exception exception, Runnable next) {
        ...
        builder.setCancelable(false);
        builder.setAlertLevel(FCLAlertDialog.AlertLevel.ALERT);
        builder.setNegativeButton(context.getString(top.newblock.fcl.R.string.dialog_positive), next::run);
        String title;
        String msg;
