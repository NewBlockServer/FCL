...
public class ModUpdatesPage extends FCLTempPage implements View.OnClickListener {
    ...
    private void exportList() {
        Path path = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/FCL-NewBlockClient", "fcl-mod-update-list-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH-mm-ss")) + ".csv").toPath();
    ...
    public static final class ModUpdateObject {
        ...
        public ModUpdateObject(Context context, LocalModFile.ModUpdate data) {
            ...
            switch (data.getCurrentVersion().getSelf().getType()) {
                case CURSEFORGE:
                    source.set(context.getString(top.newblock.fcl.R.string.mods_curseforge));
                    break;
                case MODRINTH:
                    source.set(context.getString(top.newblock.fcl.R.string.mods_modrinth));
            }
        }
